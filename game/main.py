...
Sky()
#EditorCamera()
f = open("../game/assets/worldgen/data", "r")
...
for i in range(width):
  for j in range(width):
    box = Button(color=color.white, model='cube', position=(j, 0, i), texture=f'../game/assets/textures/blocks/{blockholding}.png',parent=scene, origin_y=0.5)
    boxes.append(box)
  ...
  for box in boxes:
    if box.hovered:
      if key == 'right mouse down':
        new = Button(color=color.white, model='cube', position=box.position + mouse.normal, texture=f'../game/assets/textures/blocks/{buildingblock}.png', parent=scene, origin_y=0.5)
        boxes.append(new)
